...
import com.example.yearprogress.utils.getCurrentYearText
...
class YearProgressWidgetReceiver : GlanceAppWidgetReceiver() {
    override val glanceAppWidget: GlanceAppWidget get() = YearProgressWidget
    ...
    override fun onReceive(context: Context, intent: Intent) {
        ...
        if (
            ...
            coroutineScope.launch {
                manager.getGlanceIds(YearProgressWidget::class.java).forEach { id ->
...
object YearProgressWidget : GlanceAppWidget() {
        ...
        provideContent {
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
                ProgressWidgetContent(
                    ...
                    label = localizedContext.getString(R.string.year),
                    extraInfo = "${getCurrentYearText()} | " +
                        formatRemainingTime(
                            remainingSeconds(TimePeriod.YEAR),
                            TimePeriod.YEAR
                        )
                )
            }
